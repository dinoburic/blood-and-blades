...
public class GameUI_Manager : MonoBehaviour
{
    ...
    void Update()
    {
        if (GM.playerCharacter == null)
            return;

        if (GM.playerCharacter.GetComponent<Health>() != null)
        {
    ...
    {
        if (curretState == GameUI_State.GamePlay)
            SwitchUIState(GameUI_State.Pause);
        else if (curretState == GameUI_State.Pause)
            SwitchUIState(GameUI_State.GamePlay);
    }
